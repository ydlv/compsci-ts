...
export class Query<T> implements Iterable<T> {
    ...
    first(): T
    first<TFallback = T>(props?: { fallbackValue?: TFallback, fallbackFactory?: () => TFallback, ifNoneThrow?: () => Error } ) {
        for(const x of this) {
            return x;
        }
    ...
    single(): T
    single<TFallback = T>(props?: { fallbackValue?: TFallback, fallbackFactory?: () => TFallback, ifNoneThrow?: () => Error } ) {
        let found = false;
        let value: T | undefined = undefined;
        for(const x of this) {
            if(found) {
                throw new IllegalOperationError("single was called on a Query that has two or more elements: " + [x, value]);
            } else {
                found = true;
                value = x;
            }
