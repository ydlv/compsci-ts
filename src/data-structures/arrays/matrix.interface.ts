...
export interface Matrix<R extends number, C extends number> extends MultidimensionalArray<number, 2> {
...
export interface MutableMatrix<R extends number = number, C extends number = number> extends Matrix<R, C>, MutableMultidemsionalArray<number, 2> {
    getDimension(dim: 0): R;
    getDimension(dim: 1): C;
    readonly shape: [R, C];
